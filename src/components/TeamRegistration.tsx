...
import { UserPlus, Upload, Plus, X, Lock } from 'lucide-react';
...
export default function TeamRegistration() {
  ...
  const checkRegistrationStatus = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select('team_registration_active')
        .limit(1)
        .single();
      ...
      if (error) {
        ...
        setRegistrationActive(data.team_registration_active);
      }
    } catch (err) {
  ...
  return (
    <section id="register" className="min-h-screen py-12 sm:py-16 md:py-20 relative overflow-hidden">
      <div className="container mx-auto px-4 sm:px-6 relative z-10">
        <div className="text-center mb-8 sm:mb-12 md:mb-16 animate-fade-in">
          ...
          <p className="text-gray-400 text-sm sm:text-base md:text-lg">Join the competition</p>
        </div>
        ...
        <div className="max-w-3xl mx-auto bg-gray-900/50 border border-blue-500/20 rounded-xl p-4 sm:p-6 md:p-8 animate-slide-up glow-box relative">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5 md:space-y-6" style={{ opacity: registrationActive || checkingStatus ? 1 : 0.3 }}>
            <div>
              <label className="block text-gray-300 mb-1.5 sm:mb-2 font-semibold text-sm sm:text-base">Team Name *</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-gray-300 mb-1.5 sm:mb-2 font-semibold text-sm sm:text-base">Team Captain *</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-gray-300 mb-1.5 sm:mb-2 font-semibold text-sm sm:text-base">Team Members</label>
              <div className="space-y-2 sm:space-y-3">
                ...
                <button
                  type="button"
                  onClick={addMember}
                  className="w-full py-2 sm:py-2.5 md:py-3 bg-blue-600/20 border border-blue-500/50 text-blue-400 rounded-lg text-sm sm:text-base font-semibold hover:bg-blue-600/30 transition-all flex items-center justify-center space-x-2"
                >
            ...
            <div>
              <label className="block text-gray-300 mb-1.5 sm:mb-2 font-semibold text-sm sm:text-base">Team Photo</label>
              {previewUrl ? (
                <div className="relative">
                  <img
                    src={previewUrl}
                    alt="Team photo preview"
                    className="w-full h-40 sm:h-48 object-cover rounded-lg mb-2 border border-blue-500/30"
                  />
                  <button
                    type="button"
                    onClick={removeImage}
                    className="absolute top-2 right-2 p-1.5 sm:p-2 bg-red-600/80 text-white rounded-full hover:bg-red-600 transition-all"
                  >
                    <X className="w-3 h-3 sm:w-4 sm:h-4" />
                  </button>
                </div>
              ) : (
                <label className="flex flex-col items-center justify-center w-full h-24 sm:h-32 border-2 border-blue-500/30 border-dashed rounded-lg cursor-pointer bg-black/50 hover:bg-black/70 transition-all">
                  <div className="flex flex-col items-center justify-center pt-3 pb-4 sm:pt-5 sm:pb-6">
                    <Upload className="w-6 h-6 sm:w-8 sm:h-8 mb-1.5 sm:mb-2 text-blue-400" />
                    <p className="mb-1 sm:mb-2 text-xs sm:text-sm text-gray-400">
                      <span className="font-semibold text-blue-400">Click to upload</span> <span className="hidden sm:inline">or drag and drop</span>
                    </p>
            ...
            <div className="grid md:grid-cols-2 gap-4 sm:gap-5 md:gap-6">
              <div>
                <label className="block text-gray-300 mb-1.5 sm:mb-2 font-semibold text-sm sm:text-base">Facebook</label>
                <input
                  type="text"
                  value={formData.fb}
              ...
              <div>
                <label className="block text-gray-300 mb-1.5 sm:mb-2 font-semibold text-sm sm:text-base">Contact Number *</label>
                <input
                  type="tel"
